...
def evolution_step(resident_surface, fitness_function, mutation_function, atol, *args, **kwargs):
    ...
    """
    invasion = False
    mutant = mutation_function(resident_surface, .1, **kwargs)
...
def _number_of_generations_in_summary_dict(summary_ordered_dict):
    ...
    """
    suma = 0
    for i in range(min(summary_ordered_dict.keys()), max(summary_ordered_dict.keys()) + 1):
...
def _expand_compact_time_series(series_compact, record_every):
    ...
    # keys_list = list(working_copy.keys())
    key_top = list(working_copy.keys())[0]
