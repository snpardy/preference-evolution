...
def example_evolution():
    # the x and y axes
    # x is my pay-off and y is my opponent - so, we begin with Z=X
    ...
    initial_surface = [X,Y, (deepcopy(X))/10]
    # result_surface, time_series = p.evolve(initial_surface, p.distance_fitness_function, p.gaussian_mutation, 1000, target_surface=target_surface,
    ...
    ax1.plot_surface(X, Y, target_surface[2],cmap='viridis',linewidth=0)
    ...
    ax2.plot_surface(X, Y, initial_surface[2],cmap='viridis',linewidth=0)
